// generate-tokens.js
// Generate tokens from lesson files
...
for (const lang of uniqueLangs) {
  try {
    ...
    if (Prism.languages[lang]) {
      ...
      console.warn(
        `\x1b[33m[SKIPPED]\x1b[0m Language '${lang}' is not recognized by Prism.js after loading.`,
      );
    }
  } catch {
    console.warn(
      `\x1b[33m[SKIPPED]\x1b[0m Failed to load unsupported language: ${lang}`,
    );
  }
}
...
await Promise.all(
  lessons.map(async (lesson) => {
    if (!supportedLangs.includes(lesson.language)) {
      console.warn(
        `\x1b[33m[SKIPPED]\x1b[0m ${lesson.slug} — unsupported language: ${lesson.language}`,
      );
      return;
    }
